...
import static frc.robot.Constants.TeleopDriveConstants.MAX_DEMO_TELEOP_ANGULAR_VELOCITY;
import static frc.robot.Constants.TeleopDriveConstants.MAX_DEMO_TELOP_VELOCITY;
...
public class XBoxDemoControlBindings extends ControlBindings {
  ...
  public Supplier<LinearVelocity> translationX() {
    return () -> translationX.mut_replace(
        MAX_DEMO_TELOP_VELOCITY.in(MetersPerSecond) * -squareAxis(driverController.getLeftY()),
          MetersPerSecond);
  }
  ...
  public Supplier<LinearVelocity> translationY() {
    return () -> translationY.mut_replace(
        MAX_DEMO_TELOP_VELOCITY.in(MetersPerSecond) * -squareAxis(driverController.getLeftX()),
          MetersPerSecond);
  }
  ...
  public Supplier<AngularVelocity> omega() {
    return () -> omega.mut_replace(
        MAX_DEMO_TELEOP_ANGULAR_VELOCITY.in(RadiansPerSecond) * -squareAxis(driverController.getRightX()),
          RadiansPerSecond);
  }
